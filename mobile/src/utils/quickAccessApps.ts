...
//   - Persistencia: no hay `localStorage` en React Native, así que aquí se usa `expo-secure-store`
//     (igual que auth/storage.ts) en vez de reinventar otro mecanismo. SecureStore solo admite
//     claves alfanuméricas + ".", "-", "_" (nada de ":"), así que las claves cambian de
//     "life-organizer:quick-access" a "life-organizer.quick-access" — ver el comentario en
//     auth/storage.ts sobre por qué ":" hace que SecureStore lance en cuanto se llama.
//   - Abrir la app/web: no hace falta el truco de `visibilitychange` que usa la web para adivinar
//     si el esquema de la app abrió algo (ahí no existe una API de "¿está instalada?") — en
//     nativo, `Linking.openURL` ya rechaza la promesa si nada puede manejar el esquema, así que
//     el fallback a la web es un simple `catch`.
...
const SELECTED_IDS_KEY = "life-organizer.quick-access";
export const DEFAULT_QUICK_ACCESS_IDS = ["whatsapp", "spotify", "github", "gitlab", "gmail", "teams"];

export async function loadSelectedIds(): Promise<string[]> {
  const raw = await SecureStore.getItemAsync(SELECTED_IDS_KEY);
  if (!raw) return DEFAULT_QUICK_ACCESS_IDS;
  try {
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? parsed.filter((id): id is string => typeof id === "string") : DEFAULT_QUICK_ACCESS_IDS;
  } catch {
    return DEFAULT_QUICK_ACCESS_IDS;
  }
}

export async function saveSelectedIds(ids: string[]): Promise<void> {
  await SecureStore.setItemAsync(SELECTED_IDS_KEY, JSON.stringify(ids));
}

// --- Enlaces personalizados -------------------------------------------------------------------
const CUSTOM_LINKS_KEY = "life-organizer.quick-access-custom";
...
export async function loadCustomLinks(): Promise<CustomQuickAccessLink[]> {
  const raw = await SecureStore.getItemAsync(CUSTOM_LINKS_KEY);
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
...
async function saveCustomLinks(links: CustomQuickAccessLink[]): Promise<void> {
  await SecureStore.setItemAsync(CUSTOM_LINKS_KEY, JSON.stringify(links));
}

// Colores fijos rotando por hash del id (no por posición) para que el color de un enlace no
// cambie al borrar o añadir otros — igual que en la web.
...
// Antepone https:// si falta esquema. A diferencia de la web, no usamos `new URL(...)` para
// validar (Hermes no trae el global `URL` sin un polyfill que este proyecto no tiene) — una
// expresión regular basta para el único caso que nos importa: rechazar cualquier cosa que no sea
// http/https (p.ej. un esquema tipo "javascript:").
...
export async function removeCustomLink(id: string): Promise<void> {
  await saveCustomLinks((await loadCustomLinks()).filter((link) => link.id !== id));
}
...
export async function openQuickAccessApp(app: QuickAccessApp): Promise<void> {
  if (app.appUrl) {
    try {
      await Linking.openURL(app.appUrl);
      return;
    } catch {
      // Nada pudo abrir ese esquema (app no instalada, o el SO ni lo reconoce) — seguimos a la web.
    }
  }
